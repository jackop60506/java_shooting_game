...
public class Enemy_Rocket extends Enemy{
	private ImageSequence_enemy ims_enemy;
	public Clip_rocket_enemy ce;
	...
	public static final int ROCKET_LIFE=5;
	...
	public Enemy_Rocket(){
		...
		try{
			img= ImageIO.read(new File("pic/enemy_2_0.png"));
		}catch(Exception e){
	...
	public void display(Graphics g){
		...
		at.rotate(angle);
		//at.rotate(model.getAngle());
		//at.translate(-(model.get_W()/2),0.0);
		at.scale(0.3,0.3);
		at.translate(-model.get_W()*1.5,-model.get_H()*1.5);
		...
		//g2.fillRect((int)model.get_W()-(int)model.get_X(),(int)model.get_H()-(int)model.get_Y()-10,model.getLife()*(int)(model.get_W()/life),(int)(model.get_H()/10));		
	...
	public void CantAttack(boolean t){
		...
		case 1:
			if(t==true){
				...
				img=model.getimsEnmey().nextRocketsenemyImage();
			}else{
				model.setCantattack_CD(0);
				model.setAction_state("NotHurt");
				
				img=model.getimsEnmey().getRocketsenemyImage(0);
			}
			break;
		case 0:
			if(t==true){
				model.setCantattack_CD(model.getCantattack_CD()+CANTATTACK_CD_SEP);
				img=model.getimsEnmey().nextRocketenemyImage();
			}else{
				model.setCantattack_CD(0);
				model.setAction_state("NotHurt");
				
				img=model.getimsEnmey().getRocketenemyImage(0);
			}
			break;
