...
public class Enemy_normal extends Enemy {
	...
	public Enemy_normal(){
		...
		for(int i=0;i<CLIP_NUMBER;i++){
			clip_list.add(new Clip_normal_enemy());
		}
		...
		try{
			img= ImageIO.read(new File("pic/enemy_1_0.png"));
		}catch(Exception e){
	...
	public void display(Graphics g){
		...
		if(model.get_current_state().equals("Died")){
			//item_power.getItemState().setArea_X(200);
			//item_power.getItemState().setArea_Y(300);
			...
			switch((int)chance){
				case 1:
					if(t==true){
						...
						img=model.getimsEnmey().nextNormalsenemyImage();
					}else{
						model.setCantattack_CD(0);
						model.setAction_state("NotHurt");
						
						img=model.getimsEnmey().getNormalsenemyImage(0);
					}
					break;
				case 0:
					if(t==true){
						model.setCantattack_CD(model.getCantattack_CD()+CANTATTACK_CD_SEP);
						img=model.getimsEnmey().nextNormalenemyImage();
					}else{
						model.setCantattack_CD(0);
						model.setAction_state("NotHurt");
						
						img=model.getimsEnmey().getNormalenemyImage(0);
					}
					break;
