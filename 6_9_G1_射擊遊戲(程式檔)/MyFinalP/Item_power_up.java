...
public class Item_power_up extends Item{
...
public Item_power_up (){
		...
		try{
			Item= ImageIO.read(new File("pic/power_upitem.png"));
	...
	public void display(Graphics g){
		...
		if(model.get_current_state().equals("flying")){
			...
			if(Image_sep>5.0*Image_sep_temp){
				Item=ims_enemy.nextpowerupImage();
				Image_sep_temp++;
			}
	...
	public void Specialtoplane(Player p){
			...
			p.shoot_normal_upgrade();
			p.PlayerBulletCDClear();
